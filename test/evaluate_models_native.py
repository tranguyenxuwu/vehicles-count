#!/usr/bin/env python3
"""
YOLO Model Evaluation on UAVDT-Preview Dataset (Native Inference)
=================================================================
...
Uses standard YOLO inference (no SAHI slicing). Each model runs at its
native resolution:
  • yolo26m    → 960 px
  • yolo26m-p2 → 768 px
...
Usage:
    ...
    python evaluate_models_native.py [--conf 0.5] [--iou 0.5]
"""
...
def run_evaluation(model_name: str, model_path: str, dataset: list[dict],
    ...
    """Run full evaluation for one model using native YOLO inference."""
    print(f"\n{'='*70}")
    print(f"  Evaluating: {model_name}  (Native)")
    ...
    for img_idx, sample in enumerate(dataset):
        ...
        if results and len(results) > 0:
            ...
            if boxes is not None and len(boxes) > 0:
                for i in range(len(boxes)):
                    ...
                    eval_cls = map_prediction_class(cls_id, cls_id_to_eval)
                    if eval_cls:
        ...
        # ── Progress ──
        if (img_idx + 1) % 50 == 0 or img_idx == len(dataset) - 1:
    ...
    print(f"  │  Inference:      Native (no SAHI)             │")
    print(f"  └──────────────────────────────────────────────┘")
...
def generate_comparison_table(all_results: list[dict]) -> str:
    ...
    lines = []
    lines.append("# YOLO Model Comparison on UAVDT-Preview (Native Inference)")
    lines.append(f"\nDate: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"Dataset: UAVDT-Preview (train + test splits combined)")
    lines.append(f"Inference: Native YOLO (no SAHI slicing)")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate YOLO models on UAVDT-Preview dataset (Native)")
    ...
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  YOLO Model Evaluation on Visdrone_test  (Native Inference) ║")
    print("╚══════════════════════════════════════════════════════════════╝")
    ...
    for result in all_results:
        out_path = RESULTS_DIR / f"{result['model']}_native_{timestamp}.json"
        with open(out_path, "w") as f:
            json.dump(result, f, indent=2)
        print(f"\n  Saved: {out_path}")

    # Comparison JSON
    comparison_path = RESULTS_DIR / f"comparison_native_{timestamp}.json"
    ...
    report_path = RESULTS_DIR / f"report_native_{timestamp}.md"
    ...
    # ── Final comparison ──
    print(f"\n{'='*70}")
    print(f"  FINAL COMPARISON  (Native Inference)")
    print(f"{'='*70}")
